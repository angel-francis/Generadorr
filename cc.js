...
/*function Gen(){
    ...
    while(num>pass.length){
    pass += numeros[Math.floor(Math.random() * numeros.length)];
    cc.innerHTML = `cc-${entrada.value}${pass} / ${cvv.value}`; 
    }
...
let otro = [];
  ...
  //numero = Array.from({length: 999}, (_, i) => i.toString().padStart(3, '0'));
...
/*entrada.addEventListener('input',(e)=>{
    if(isNaN(e.target.value)){
        mensaje.innerHTML = "solo se aceptan numeros";
         ...
         mensaje.innerHTML = "No se aceptan menos de 16 digitos";
    }else if(e.target.value === 16){
         mensaje.innerHTML = "";
    }else{
        mensaje.innerHTML = "";
...
let c = "5043671405584125";
let fe = "26"
numero.forEach(ele =>{
    //ul.innerHTML += `${c} / ${ele} /  ${fe}  `;
    //console.log(`${c} / ${ele} /  ${fe}---`)
    otro.push(`${c} / ${ele} / ${fe}`);
})

otro.forEach((k, index)=>{
    ul.innerHTML += `<li>${k}</li> `;
})*/
